...
k8s_resource(
  ...
  new_name='database',
  objects=['cncfdemo-postgres-cluster'],
  extra_pod_selectors=[{'cnpg.io/cluster': 'cncfdemo-postgres-cluster'}],
  labels='app',
  port_forwards=5432)
...
deployment_create(
  ...
  envFrom=[{
    "secretRef":{
      "name": "cncfdemo-postgres-cluster-app" # autogenerated when creating a postgres cluster without any initdb arguments
    }
  }]
)
